...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
        h_next, cache[i] = rnn_step_forward(x[:, i, :], h[:, i, :], Wx, Wh, b)
        h = torch.cat((h, h_next.unsqueeze(1)), dim=1)
    h = h[:, 1:, :]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_backward(dh, cache):
    ...
    dh0 = torch.zeros_like(dh[:, 0, :])

    for i in reversed(range(T)):
        step_dh = dh[:, i, :] + dh0
        step_dx, dh0, step_dWx, step_dWh, step_db = rnn_step_backward(step_dh, cache[i])
        dx[:, i, :] += step_dx
        dWx += step_dWx
        dWh += step_dWh
        db += step_db
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            ...
            h0 = self.feature_projection(A.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)

        else:
            # Define Adaptive Average Pooling layer
            pool = nn.AdaptiveAvgPool2d((1, 1))

            # Apply the pooling
            pooled_tensor = pool(self.image_encoder(images))

            # Reshape to get the desired shape [-1, 400]
            output_tensor = pooled_tensor.view(pooled_tensor.size(0), -1)

            h0 = self.feature_projection(output_tensor)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            A = self.image_encoder(images)
            A = self.feature_projection(A.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            h0 = A.mean(dim=(2, 3))
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        f = torch.sigmoid(A[:, prev_h.shape[1] : prev_h.shape[1] * 2])
        o = torch.sigmoid(A[:, prev_h.shape[1] * 2 : prev_h.shape[1] * 3])
        g = torch.tanh(A[:, prev_h.shape[1] * 3 :])
        next_c = f * prev_c + i * g
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for T in range(x.shape[1]):
            hn_next, cn_next = self.step_forward(x[:, T, :], hn[:, T, :], cn[:, T, :])
            hn = torch.cat((hn, hn_next.unsqueeze(1)), dim=1)
            cn = torch.cat((cn, cn_next.unsqueeze(1)), dim=1)
        hn = hn[:, 1:, :]
        cn = cn[:, 1:, :]
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    A_flat = A.view(N, H, -1)
    M = prev_h.unsqueeze(1).bmm(A_flat).div(math.sqrt(H))

    attn_weights = torch.softmax(M, dim=2)
    attn = attn_weights.bmm(A_flat.transpose(1, 2)).squeeze(1)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for T in range(x.shape[1]):
            attn, _ = dot_product_attention(hn[:, T, :], A)
            hn_next, cn_next = self.step_forward(x[:, T, :], hn[:, T, :], cn[:, T, :], attn)
            hn = torch.cat((hn, hn_next.unsqueeze(1)), dim=1)
            cn = torch.cat((cn, cn_next.unsqueeze(1)), dim=1)
        hn = hn[:, 1:, :]
        cn = cn[:, 1:, :]
