...
def softmax_loss_naive(
    ...
    for i in range(X.shape[0]):
        ...
        softmax = scores_exp / torch.sum(scores_exp)
        ...
        # Compute gradient
        dloss = 1
        dscores = softmax * dloss
        for j in range(num_classes):
            if j == y[i]:
                dW[:, j] += ((dscores[j] - 1)/ X.shape[0]) * X[i]  # for correct class
            else:
                dW[:, j] += (dscores[j]/ X.shape[0]) * X[i]  # for incorrect classes
...
def softmax_loss_vectorized(
    ...
    # regularization!                                                           #
    #############################################################################
    # Compute scores matrix of shape (N,C) for each sample where each column
    # corresponds to the class score of that sample
    scores = X.mm(W)
    ...
    scores_exp = torch.exp(scores)
    softmax = scores_exp / torch.sum(scores_exp, dim=1, keepdim=True)
    loss = torch.mean(-torch.log(softmax[torch.arange(X.shape[0]), y]))

    # Compute gradient
    softmax[torch.arange(X.shape[0]), y] -= 1
    dloss = 1
    dscores = (softmax / X.shape[0]) * dloss
    dW = X.t().mm(dscores)
