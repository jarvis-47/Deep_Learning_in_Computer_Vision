...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        x_flat = x_flat.to(device=w.device, dtype=w.dtype)
        out = x_flat.mm(w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params = {k: v.to(device = device, dtype = dtype) for k, v in self.params.items()}
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        ###################################################################
        # Compute the loss and gradient with softmax 
        loss, dS = softmax_loss(scores, y)
        
        # Add regularization to the loss
        loss += self.reg * (torch.sum(self.params['W1'] ** 2) + torch.sum(self.params['W2'] ** 2))

        # Compute the gradient of loss w.r.t. W2 and b2
        grads['W2'] = h1.t().mm(dS) + 2 * self.reg * self.params['W2']
        grads['b2'] = torch.sum(dS, axis=0)

        # Compute the gradient of loss w.r.t. h1
        dh1 = dS.mm(self.params['W2'].t())

        # Compute the gradient of loss w.r.t. W1 and b1
        grads['W1'] = X.t().mm(dh1 * (h1 > 0)) + 2 * self.reg * self.params['W1']
        grads['b1'] = torch.sum(dh1 * (h1 > 0), axis=0)
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            self.params['W' + str(i)] = torch.randn(input_dim if i == 1 else hidden_dims[i - 2],
                                                    num_classes if i == self.num_layers
                                                      else hidden_dims[i - 1]) * weight_scale
            self.params['b' + str(i)] = torch.zeros(num_classes if i == self.num_layers
                                                      else hidden_dims[i - 1])
            self.params['W' + str(i)] = self.params['W' + str(i)].to(device = device, dtype = dtype)
            self.params['b' + str(i)] = self.params['b' + str(i)].to(device = device, dtype = dtype)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            if i == 1:
                h, cache = Linear_ReLU.forward(X, self.params['W' + str(i)], self.params['b' + str(i)])
            elif i == self.num_layers:
                scores, cache = Linear.forward(h, self.params['W' + str(i)], self.params['b' + str(i)])
            else:
                h, cache = Linear_ReLU.forward(h, self.params['W' + str(i)], self.params['b' + str(i)])
            ...
            if self.use_dropout and i != self.num_layers:
                h, dropout_cache = Dropout.forward(h, self.dropout_param)
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                dh, grads['W' + str(i)], grads['b' + str(i)] = Linear.backward(dS, cache)
            else:
                if self.use_dropout:
                    dropout_cache = dropout_cache_list.pop()
                    dh = Dropout.backward(dh, dropout_cache)
                dh, grads['W' + str(i)], grads['b' + str(i)] = Linear_ReLU.backward(dh, cache)
