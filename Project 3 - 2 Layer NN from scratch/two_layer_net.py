...
def nn_forward_backward(
    ...
    y: Optional[torch.Tensor] = None,
    reg: float = 0.0,
    loss_func: str = None  # Implemented both LinearSVM and Softmax Classification loss
):
    """
    ...
    if loss_func == 'svm':
        ...
        # To control numeric unstability, shifted scores of each iteration 
        # between interval [-inf, 0]   
        S_max,_ = torch.max(scores, axis=1)
        
        # Computed predicted probabilities
        S_exp = torch.exp((scores.T - S_max).T)
        S_pred = (S_exp.T / torch.sum(S_exp, axis=1)).T 
        
        # Computed Softmax Classifier loss    
        loss = torch.mean(-torch.log(S_pred[range(N), y]))
    ...
    if loss_func == 'svm':
        ...
        # Gradient of log_softmax wrt scores: 
        # { S_pred - 1 --> For correct class}
        # { S_pred     --> otherwise        }
        
        dscores = dloss * S_pred
        dscores[torch.arange(N), y] -= 1
        dscores /= N
    
    # Computed 2nd FC layer gradients
    grads['b2'] = torch.sum(dscores, dim=0)
    grads['W2'] = h1.t().float().mm(dscores.float())
    
    # Computed gradients to be passed to 1st FC layer
    grad_h = dscores.float().mm(W2.t().float())
    grad_h[(X.mm(W1)+b1) < 0] = 0  #Backprop for ReLU activation 
    
    # Computed 1st FC layer gradients
    grads['b1'] = torch.sum(grad_h, dim=0)
    grads['W1'] = X.t().float().mm(grad_h.float())
...
def nn_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [1, 1.25]
    hidden_sizes = [788, 464 , 820]
...
def find_best_net(
    ...
    #############################################################################
    # Get hyperparameters from get_param_set_fn() callable function
    learning_rates, hidden_sizes, regularization_strengths, learning_rate_decays =  get_param_set_fn()
    
    # Total number of hyperparameter combinations to be tested to find best model
        ...
        for h in hidden_sizes:
            for reg in regularization_strengths:
                for lrd in learning_rate_decays:
                    ...
                    i += 1
                    print('Training NN %d / %d with lr=%e, h=%d, reg=%e and lrd=%f'% (i, num_models, lr,h, reg, lrd))
                    
                    # Class TwoLayerNet instance created for each iteration
                    net = TwoLayerNet(3 * 32 * 32, h, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
                    
                    # Called .train method of class TwoLayerNet to get stats for each iteration of hyperparameter combinations
                    stats = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                    num_iters=3100, batch_size=908, learning_rate=lr, learning_rate_decay=lrd ,reg=reg)
                    
                    # Get validation and training accuracy for the current iteration
                    #cand_val_acc = stats["val_acc_history"][-1]
                    cand_train_acc = stats["train_acc_history"][-1]
                    
                    # Predict on the validation set
                    y_val_pred = net.predict(data_dict['X_val'])
                    cand_val_acc = 100.0 * (y_val_pred == data_dict['y_val']).float().mean().item()
                    
                    # Get the best validation accuracy and hence the best model
                    if cand_val_acc > best_val_acc:
                        best_val_acc = cand_val_acc
                        best_stat = stats # save the svm
                        best_net = net
                    results[(lr, h, reg, lrd)] = (cand_train_acc, cand_val_acc)


    # Print out results.
    for lr, h, reg, lrd in sorted(results):
        train_acc, val_acc = results[(lr, h, reg, lrd)]
        print('lr %e h %d reg %e lrd %f train accuracy: %f val accuracy: %f' % (
            lr, h, reg, lrd, train_acc, val_acc))
    
    print('best validation accuracy achieved during cross-validation: %.2f%%' % best_val_acc)
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
