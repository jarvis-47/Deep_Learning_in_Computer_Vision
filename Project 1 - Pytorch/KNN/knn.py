...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = x_train[i].flatten().sub(x_test[j].flatten()).pow(2).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    for i in range(num_train):
        dists[i] = x_train[i].flatten().sub(x_test.reshape(num_test,-1)).pow(2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Compute the squared L2 distances using broadcasting
    x_train_sq = torch.sum(x_train_flat ** 2, dim=1, keepdim=True) 
    x_test_sq = torch.sum(x_test_flat ** 2, dim=1, keepdim=True)

    # Compute the pairwise distance matrix using broadcasting
    dists = x_train_sq - 2 * x_train_flat.mm(x_test_flat.T) + x_test_sq.T  
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    for j in range(num_test):
        topk = torch.topk(dists[:,j], k, largest=False, sorted=True)
        y_pred[j] = y_train[topk.indices].mode().values.item()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        k_to_acc_avg[k] = sum(k_to_accuracies[k])/len(k_to_accuracies[k])
        print(f'k = {k}, accuracy = {k_to_acc_avg[k]}')
        if best_k == 0:
            best_k = k
        for key, value in k_to_acc_avg.items():
            if value > k_to_acc_avg[best_k]:
                best_k = key
            elif value == k_to_acc_avg[best_k]:
                best_k = min(key, best_k)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
