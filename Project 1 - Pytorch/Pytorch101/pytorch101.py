...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for i in range(start, stop + 1):
        if i % 10 == 0:
            x = torch.cat((x, torch.tensor([i], dtype=torch.float64)))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    #                      TODO: Implement this function                     #
    ##########################################################################
    idx0 = torch.arange(x.shape[0] - 1, -1, -1)
    y = x[idx0]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    y = torch.tensor(list(map(torch.sum, xs)))/ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    mask = torch.cat((torch.tensor([True]), mask))
    uniques = x_sorted[mask]
