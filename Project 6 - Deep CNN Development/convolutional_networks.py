    ...
    def forward(x, w, b, conv_param):
        ...
        for i in range(0, H_out * stride, stride):
            for j in range(0, W_out * stride, stride):
                x_slice = x_padded[:, :, i:i+HH, j:j+WW]
                for k in range(F):
                    out[:, k, i//stride, j//stride] = torch.sum(x_slice * w[k], axis=(1,2,3)) + b[k]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(0, H_out * stride, stride):
            for j in range(0, W_out * stride, stride):
                ...
                for k in range(F):
                    dw[k] += torch.sum(dout[:, k, i//stride, j//stride].view(N, 1, 1, 1) * x_slice, axis=0)
                    dx_padded[:, :, i:i+HH, j:j+WW] += dout[:, k, i//stride, j//stride].view(N, 1, 1, 1) * w[k]
    ...
    def forward(x, pool_param):
        ...
        for i in range(0, H_out):
          for j in range(0, W_out):
              ...
              # Find the max value in the slice for height and width dimensions
              out[:, :, i, j] = torch.max(torch.max(x_slice, dim=2).values, dim=2).values    
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
        ...
        for i in range(0, H_out * stride, stride):
            for j in range(0, W_out * stride, stride):
                for n in range(N):
                    for c in range(C):
                        # Slice the input tensor
                        x_slice = x[n, c, i:i+pool_height, j:j+pool_width]
                        
                        # Find the index of the max value in the slice
                        max_val_idx = torch.argmax(x_slice)
                        
                        # Convert the linear index to height and width indices
                        h_idx = max_val_idx // pool_width
                        w_idx = max_val_idx % pool_width

                        # Propagate the gradient to the max position
                        dx[n, c, i+h_idx, j+w_idx] += dout[n, c, i//stride, j//stride]
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = torch.randn(num_filters * input_dims[1] * input_dims[2] // 4, hidden_dim, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # above                                                              #
        ######################################################################
        convreLUpool, convreLUpool_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        linearReLU, linearReLU_cache = Linear_ReLU.forward(convreLUpool, W2, b2)
        linear, linear_cache = Linear.forward(linearReLU, W3, b3)
        scores = linear
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dlinear, grads['W3'], grads['b3'] = Linear.backward(dS, linear_cache)
        dlinearReLU, grads['W2'], grads['b2'] = Linear_ReLU.backward(dlinear, linearReLU_cache)
        dconvreLUpool, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dlinearReLU, convreLUpool_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        max_pool_enc = torch.zeros(self.num_layers-1, dtype=torch.int)
        max_pool_enc[max_pools] = 1

        self.max_pool_enc = max_pool_enc

        for i in range(1, self.num_layers):
            if i == 1:
                if weight_scale == 'kaiming':
                  self.params['W' + str(i)] = kaiming_initializer(C, num_filters[i-1], K=3, device=device, dtype=dtype)
                else:
                  self.params['W' + str(i)] = torch.randn(num_filters[i-1], C, 3, 3, dtype=dtype, device=device) * weight_scale
                self.params['b' + str(i)] = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
            else:
                if weight_scale == 'kaiming':
                  self.params['W' + str(i)] = kaiming_initializer(num_filters[i-2], num_filters[i-1], K=3, device=device, dtype=dtype)
                else:
                  self.params['W' + str(i)] = torch.randn(num_filters[i-1], num_filters[i-2], 3, 3, dtype=dtype, device=device) * weight_scale
                self.params['b' + str(i)] = torch.zeros(num_filters[i-1], dtype=dtype, device=device)
            ...
            if self.max_pool_enc[i-1] == 1:
                    H = H // 2
                    W = W // 2
        if weight_scale == 'kaiming':
            self.params['W' + str(self.num_layers)] = kaiming_initializer(num_filters[-1] * H * W, num_classes, device=device, dtype=dtype)
        else:
          self.params['W' + str(self.num_layers)] = torch.randn(num_filters[-1] * H * W, num_classes, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.batchnorm:
                ...
                if self.max_pool_enc[i-1] == 1:
                  crp[i], crp_cache[i] = Conv_BatchNorm_ReLU_Pool.forward(crp[i-1], W, b, gamma, beta, conv_param, self.bn_params[i-1], pool_param)
                else:
                  crp[i], crp_cache[i] = Conv_BatchNorm_ReLU.forward(crp[i-1], W, b, gamma, beta, conv_param, self.bn_params[i-1])
            else:
                if self.max_pool_enc[i-1] == 1:
                  crp[i], crp_cache[i] = Conv_ReLU_Pool.forward(crp[i-1], W, b, conv_param, pool_param)
                else:
                  crp[i], crp_cache[i] = Conv_ReLU.forward(crp[i-1], W, b, conv_param)

        scores, scores_cache = Linear.forward(crp[self.num_layers-1], self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        # Add regularization to the loss.
        for i in range(1, self.num_layers):
            loss += self.reg * torch.sum(self.params['W' + str(i)] * self.params['W' + str(i)])

        # Backward pass: compute gradients
        dcrp = {}
        dcrp_cache = {}
        dcrp[self.num_layers], grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = Linear.backward(dS, scores_cache)
        for i in range(self.num_layers-1, 0, -1):
            if self.batchnorm:
                if self.max_pool_enc[i-1] == 1:
                    dcrp[i], grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads['beta' + str(i)] = Conv_BatchNorm_ReLU_Pool.backward(dcrp[i+1], crp_cache[i])
                else:
                    dcrp[i], grads['W' + str(i)], grads['b' + str(i)], grads['gamma' + str(i)], grads['beta' + str(i)] = Conv_BatchNorm_ReLU.backward(dcrp[i+1], crp_cache[i])
            else:
                if self.max_pool_enc[i-1] == 1:
                    dcrp[i], grads['W' + str(i)], grads['b' + str(i)] = Conv_ReLU_Pool.backward(dcrp[i+1], crp_cache[i])
                else:
                    dcrp[i], grads['W' + str(i)], grads['b' + str(i)] = Conv_ReLU.backward(dcrp[i+1], crp_cache[i])
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                      num_filters=[8, 32, 32, 128, 128, 128],
                      max_pools=[3, 4, 5],
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x, x_normalized, gamma, sample_mean, sample_var, eps, bn_param)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        x, x_normalized, gamma, sample_mean, sample_var, eps, bn_param = cache
        mode = bn_param['mode']
        N, D = x.shape

        # Gradient of loss w.r.t. output of BN layer
        ...
        if mode == 'train':
          # Gradient w.r.t. sample variance and mean
          dsample_var = torch.sum(dx_normalized * (x - sample_mean) * -0.5 * torch.pow(sample_var + eps, -1.5), dim=0)
          dsample_mean = torch.sum(dx_normalized * -1 / torch.sqrt(sample_var + eps), dim=0)
          dx = dx_normalized / torch.sqrt(sample_var + eps)
          dx += dsample_var * 2 * (x - sample_mean) / N
          dx += dsample_mean / N

        elif mode == 'test':
          dx = dx_normalized / torch.sqrt(bn_param['running_var'] + eps)
         #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        ###################################################################
        x, x_hat, gamma, sample_mean, sample_var, eps, bn_param = cache
        N, D = x.shape

        # Step 2: Gradients w.r.t gamma and beta
        dbeta = dout.sum(dim=0)
        dgamma = (dout * x_hat).sum(dim=0)
        
        # Step 3: Gradient w.r.t x_hat
        dx_hat = dout * gamma
        
        # Intermediate gradients
        ivar = torch.rsqrt(x.var(dim=0, keepdim=True) + eps)
        x_mu = x - x.mean(dim=0, keepdim=True)
        
        # Gradient w.r.t x using simplified computations
        dx = (1.0 / N) * ivar * (N * dx_hat - dx_hat.sum(dim=0) - x_hat * (dx_hat * x_hat).sum(dim=0))
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Reshape the input to (N*H*W, C) so that we can use vanilla batch normalization
        x_reshaped = x.permute(0, 2, 3, 1).contiguous().view(-1, C)
        
        # Use BatchNorm's forward
        out_reshaped, cache = BatchNorm.forward(x_reshaped, gamma, beta, bn_param)
        
        # Reshape the normalized output back to original shape
    ...
    def backward(dout, cache):
        ...
        # Reshape dout similarly to what we did in the forward pass
        dout_reshaped = dout.permute(0, 2, 3, 1).contiguous().view(-1, C)
        
        # Use BatchNorm's backward
